...
class Articles extends Translatable
{
	...
	public function tableName()
	{
        ...
        public function scopes(){
            ...
            return array(
                ...
                'default'=>array(
                    'condition'=>"$t.active=1 and $t.deleted=0",
                    'order'=>"$t.sort asc, $t.date desc"
                ),
                'articles'=>array(
                    'condition'=>"$t.type!='static'"
                )
            );
        }
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, parent_id, type,pic_name, date,modify_date,create_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
                        ...
                        'photos'=>array(self::HAS_MANY,'Gallery','parent_id','condition'=>'photos.type="Articles"'),
                        'parent'=>array(self::BELONGS_TO,'Menus','parent_id'),
                       'cleanurls'=>array(self::HAS_MANY,'Cleanurls','parent_id','condition'=>'cleanurls.type="Articles"','together'=>true,'index'=>'language')
		);
	}
        ...
        public function getActions(){
                $tmp = array('content'=>array(
                            ...
                            'delete'=>array('title'=>($this->deleted?'Undelete':'Delete').' item',
                                            'params'=>array('aclass'=>'magnum-'.($this->deleted?'undelete':'delete').' magnum-ajax', 'class'=>'glyphicon glyphicon-trash', 'url'=> 'articles/delete','data'=>array('id'=>$this->id))),
