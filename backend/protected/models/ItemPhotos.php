...
class ItemPhotos extends Translatable
{
	...
	public function rules()
	{
		...
		return array(
                        array('type','default','value'=>0,'setOnEmpty'=>true),
			array('color_id, item_id,type', 'numerical', 'integerOnly'=>true),
	...
	public function attributeLabels()
	{
        ...
        public function getTypes(){
            return array(
                '0'=>Utilities::t('Photos'),
                '1'=>  Utilities::t('Schemas')
            );
        }
	/**
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function rotate(){
            ...
            $image = Yii::app()->image->load($baseUrl.'/uploads/items/'.$this->pic_name);
            $sizes = Modules::model()->findByAttributes(array('name'=>'items'))->imagesizes;
            foreach($sizes as $size){
                @unlink($baseUrl.'/site/uploads/items/'.$size->name.'/'.$this->pic_name);
            }
            $image->rotate(90);
            $image->save();
        }
